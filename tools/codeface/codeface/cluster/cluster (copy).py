...
def computeSnapshotCollaboration(file_commit, cmtList, id_mgr, link_type,
    ...
    for cmt in revCmts:
        ...
        print("commit snapshot: " + str(cmt.id))
...
def computeCommitCollaboration(codeBlks, cmt, id_mgr, link_type, maxDist,
    ...
    #get the person responsible for this revision (d2)
    if author:
        revPerson = id_mgr.getPI( revCmtBlks[0].authorId )
    else:
        revPerson = id_mgr.getPI( revCmtBlks[0].committerId )

    print("--- revision person: " + str(revPerson))

    #find all other commit ids for older revisions (by all other developers)
    oldCmtIdSet = set( [blk.cmtHash for blk in codeBlks
                        if blk.cmtHash != cmt.id] )
    
    #keep the first occurence of each older commit
    #and revert the list to iterate commits starting
    #from the oldest one
    newlist = []
    for i in codeBlks:
        if i.cmtHash not in newlist and i.cmtHash != cmt.id:
            newlist.append(i.cmtHash)
    newlist.reverse()

    other_persons = []

    #calculate relationship between personId and all other contributors
    for oldCmtId in newlist:# oldCmtIdSet:
        print("------ commit accounted for edge weight: " + str(oldCmtId))

        #get all blocks for the oldCmtId (starting with the oldest commit, t0)
        ...
        if personId not in other_persons:
            collaboration_strength = compute_block_weight(revCmtBlks, oldRevBlks)
            #other_persons.append(personId)
        else: 
            # For the edge between the person of interest and the other person,
            # the person of interests contributed LoC have already been added,
            # so they do not have to be added another time
            print("relation has been considered")
            collaboration_strength = compute_block_weight(oldRevBlks, oldRevBlks)

        print("--------- collaboration strength: weight: " + str(collaboration_strength.weight))

        #store result
        print("--------- inEdgePerson: " + str(personId))
...
def computeBlksSize(blks1, blks2):
    ...
    for blk in blks_total:
        ...
        print("---------------- blksize:" + str(blk.end - blk.start + 1))
    return size_total
...
def compute_block_weight(blocks1, blocks2):
    ...
    if commit_ids1 == commit_ids2:
        size /= 2

    print("******************** commit_ids1" + str(commit_ids1))
    print("******************** commit_ids2" + str(commit_ids2))
